# VPC: 2 public subnets (ALB + NAT gateway) and 2 private subnets (EC2).
# Only the ALB is reachable from the internet.
...
# One NAT gateway (single AZ) - enough for a dev/small stack and half the cost of two.
...
resource "aws_security_group" "alb" {
  ...
  description = "Public entry point: HTTPS/HTTP from the internet"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "app" {
  ...
  description = "Private EC2: only the ALB may talk to Keycloak/Kafka UI"
  vpc_id      = aws_vpc.main.id
